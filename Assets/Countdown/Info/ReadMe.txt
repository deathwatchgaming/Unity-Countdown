...
This first part is added just as an example of something that can be done when the countdown completes: (Switch Scene say just for example and can be easily removed by commenting out those parts in source)

Also note: 

If you do not desire any of the debug logs to display, you can comment those out, but they are left there to help be informative while you work on things, so that is up to you to comment out or not.
...
1.A) Create an Empty Game Object called UI  to house your UI canvas (unless you already have a UI empty object to house your Canvas) * (Else You can alos instead just opt to just create a UI Canvas, up to you.)
...
TextMesh Pro - Text UI: -> Text Input: lets say 15 just for scene tab viewing display example.
...
7) You can ignore the checked "Count Down Enabled" check box unless you want to disable the CountDown
